import 'package:anabul/component/history/safekeeping/safekeeping_in_list.dart';
// import 'package:anabulhotel/component/app_bar/app_bar_tittle.dart';
// import 'package:anabulhotel/component/history/safekeeping/safekeeping_in_list.dart';
...
class SafekeepingIn extends StatefulWidget {
  const SafekeepingIn({Key? key, required this.status}) : super(key: key);
  final String status;

  @override
  State<SafekeepingIn> createState() => _SafekeepingIn();
}

class _SafekeepingIn extends State<SafekeepingIn> {
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.hasData &&
            ...
            return Container(
              child: SafeKeepingInList(
                  dataIn: historySafeKeepings, status: widget.status),
            );
          } else {
            return const Center(
                child: Text(
